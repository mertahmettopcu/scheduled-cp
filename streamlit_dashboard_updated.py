...
DEFAULT_PAIR_OPTIONS = ["BTCUSDT", "PAXGUSDT"]
...
def _read_symbols_csv_url():
    url = None
    try:
        if "google_sheets" in st.secrets:
            url = st.secrets["google_sheets"].get("symbols_csv_url")
    except Exception:
        pass
    url = url or os.getenv("GOOGLE_SHEETS_SYMBOLS_CSV_URL")
    return (url or "").strip()
...
def load_pair_options() -> list[str]:
    url = _read_symbols_csv_url()
    if not url:
        return DEFAULT_PAIR_OPTIONS

    try:
        df = pd.read_csv(url)
        df.columns = [str(c).strip().lower() for c in df.columns]
        if not {"symbol", "enabled"}.issubset(df.columns):
            return DEFAULT_PAIR_OPTIONS
        df["symbol"] = df["symbol"].astype(str).str.strip().str.upper()
        df["enabled_bool"] = df["enabled"].apply(_parse_enabled)
        pairs = df.loc[df["enabled_bool"], "symbol"].drop_duplicates().tolist()
        return pairs or DEFAULT_PAIR_OPTIONS
    except Exception:
        return DEFAULT_PAIR_OPTIONS
...
def make_ichimoku_chart(df: pd.DataFrame, title: str) -> go.Figure:
    ...
    fig.add_trace(go.Scatter(x=plot_df["display_time"], y=plot_df["tenkan"], mode="lines", name="Tenkan"))
    fig.add_trace(go.Scatter(x=plot_df["display_time"], y=plot_df["kijun"], mode="lines", name="Kijun"))
    fig.add_trace(go.Scatter(x=plot_df["display_time"], y=plot_df["chikou"], mode="lines", name="Chikou"))
...
if not snapshots.empty:
    ...
    st.dataframe(snap_view, use_container_width=True)
